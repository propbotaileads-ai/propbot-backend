...
async function sendLeadEmailToBuyer({ buyerEmail, buyerName, agentName, botName, clickToTalkUrl }) {
  const subject = `${buyerName ? buyerName + ', ' : ''}Talk to ${botName || agentName + "'s AI"}`;
...
<tr><td style="background:#0A0A0A;padding:28px 40px;">
  ...
  <p style="color:#888;font-size:12px;margin:4px 0 0;">Powered by DoorBot AI</p>
</td></tr>
<tr><td style="padding:36px 40px;">
  ...
  <p style="color:#444;font-size:15px;line-height:1.7;margin:0 0 16px;">
    Thanks for your interest! <strong>${agentName}</strong> has set up an AI assistant 
    to help you right away — available 24/7, no waiting.
  </p>
  <p style="color:#444;font-size:15px;line-height:1.7;margin:0 0 28px;">
    ...
    and book a time with ${agentName} if you'd like.
  </p>
  <table cellpadding="0" cellspacing="0" style="margin:0 auto 28px;">
    <tr><td style="background:#C9A84C;border-radius:4px;">
      <a href="${clickToTalkUrl}" style="display:block;padding:16px 36px;color:#0A0A0A;font-size:15px;font-weight:bold;text-decoration:none;">
        📞 Talk to ${botName || agentName + "'s AI"}
      </a>
    </td></tr>
  </table>
  <p style="color:#888;font-size:13px;text-align:center;">Takes less than 2 minutes • Available right now</p>
</td></tr>
<tr><td style="background:#f9f9f9;padding:20px 40px;border-top:1px solid #eee;">
  <p style="color:#aaa;font-size:12px;margin:0;text-align:center;">
    Sent on behalf of ${agentName} via DoorBot AI<br/>
    <em>We open the door, you close the deal.</em>
  </p>
</td></tr>
</table>
</td></tr>
  ...
  try {
    ...
    console.log('[DoorBot AI] Buyer email sent to:', buyerEmail);
    return true;
  } catch (err) {
...
<tr><td style="background:#0A0A0A;padding:28px 40px;">
  ...
  <p style="color:#888;font-size:12px;margin:4px 0 0;">DoorBot AI — ${new Date().toLocaleString('en-US', { timeZone: 'America/New_York' })} EST</p>
</td></tr>
<tr><td style="padding:36px 40px;">
  <p style="color:#444;font-size:15px;margin:0 0 24px;">Hi <strong>${agentName}</strong>! Your AI just booked a new appointment:</p>
  <table width="100%" cellpadding="12" cellspacing="0" style="background:#f9f9f9;border-radius:6px;margin-bottom:24px;">
    ...
    <tr><td style="border-bottom:1px solid #eee;"><strong style="color:#888;font-size:12px;text-transform:uppercase;">Property</strong><br/><span style="color:#111;font-size:15px;">${propertyType || 'N/A'}</span></td></tr>
  ...
  <p style="color:#888;font-size:13px;text-align:center;">Log in to your DoorBot AI dashboard to see full details.</p>
</td></tr>
<tr><td style="background:#f9f9f9;padding:16px 40px;border-top:1px solid #eee;">
  <p style="color:#aaa;font-size:12px;margin:0;text-align:center;">DoorBot AI — We open the door, you close the deal.</p>
</td></tr>
</table>
</td></tr>
</table>
  ...
  try {
    ...
    console.log('[DoorBot AI] Hot lead alert sent to:', agentEmail);
    return true;
  } catch (err) {
